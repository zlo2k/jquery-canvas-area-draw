(function(c){c.fn.canvasAreaDraw=function(c){this.each(function(f,d){w.apply(d,[f,d,c])})};var w=function(t,f,d){var m,l=!1,g=1;t=c.extend({imageUrl:c(this).attr("data-image-url")},d);d=c(f).val().replace(/[^0-9,]/ig,"");var b=d.length?d.split(",").map(function(a){return parseInt(a,10)}):[];d=c('<button type="button" class="btn"><i class="icon-trash"></i>\u041e\u0447\u0438\u0441\u0442\u0438\u0442\u044c</button>');var n=c('<canvas style="width: 100%">');var e=n[0].getContext("2d");var h=new Image;
var u=function(){if(n[0].clientWidth!==h.naturalWidth){g=h.naturalWidth/n[0].clientWidth;for(var a=0;a<b.length;a++)b[a]*=g}n.attr("height",h.height).attr("width",h.width);q()};c(h).load(u);h.src=t.imageUrl;h.loaded&&u();n.css({background:"url("+h.src+")"});c(f).after("<br>",n,"<br>",d);var v=function(a){a.offsetX||(a.offsetX=a.pageX-c(a.target).offset().left,a.offsetY=a.pageY-c(a.target).offset().top);b[m]=Math.round(a.offsetX)*g;b[m+1]=Math.round(a.offsetY)*g;q()};var x=function(a){a.offsetX||(a.offsetX=
a.pageX-c(a.target).offset().left,a.offsetY=a.pageY-c(a.target).offset().top);l||(l={x:Math.round(a.offsetX),y:Math.round(a.offsetY)});a={x:Math.round(a.offsetX),y:Math.round(a.offsetY)};for(var p=0;p<b.length;p++)b[p]=a.x-l.x+b[p],b[++p]=a.y-l.y+b[p];l=a;q()};var q=function(){e.canvas.width=e.canvas.width;r();if(!(2>b.length)){e.globalCompositeOperation="destination-over";e.fillStyle="rgb(255,255,255)";e.strokeStyle="rgb(255,20,20)";e.lineWidth=1;if(6<=b.length){var a=getCenter();e.fillRect(a.x-
